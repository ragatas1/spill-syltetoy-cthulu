...
public class NPC2Script : MonoBehaviour
{
    ...
    {
        if (logikkScript.harItem2 == true)
        {
            snakke = true;
            interact = false;
            text.SetActive(true);
        }
        else if (logikkScript.item2PlukketOpp)
        {
            snakke = false;           
        }
        else
    ...
    IEnumerator winDialogen()
    {
        ...
        spillerScript.moving = true;
        winDialog.SetActive(false);
        logikkScript.harItem2 = false;
        logikkScript.hjerteCounter = logikkScript.hjerteCounter + 1;
        snakke = false;
        portrait.SetActive(false);
        an.SetBool("klarte det 0", true);
        logikkScript.fornoyd2 = true;
    }
    void endreAnsikt()
    {
        if (dialogScript.index == 1) { portraitA.SetInteger("hvilketAnsikt", 0); }
        if (dialogScript.index == 2) { portraitA.SetInteger("hvilketAnsikt", 11); }
        if (dialogScript.index == 3) { portraitA.SetInteger("hvilketAnsikt", 13); }
        if (dialogScript.index == 4) { portraitA.SetInteger("hvilketAnsikt", 0); }
        if (dialogScript.index == 5) { portraitA.SetInteger("hvilketAnsikt", 13); }
        if (dialogScript.index == 6) { portraitA.SetInteger("hvilketAnsikt", 10); }
        if (dialogScript.index == 7) { portraitA.SetInteger("hvilketAnsikt", 3); }
        if (dialogScript.index == 8) { portraitA.SetInteger("hvilketAnsikt", 11); }
        if (winDialogScript.index == 1) { portraitA.SetInteger("hvilketAnsikt", 12); }
        if (winDialogScript.index == 2) { portraitA.SetInteger("hvilketAnsikt", 10); }
        if (winDialogScript.index == 3) { portraitA.SetInteger("hvilketAnsikt", 11); }
        if (winDialogScript.index == 4) { portraitA.SetInteger("hvilketAnsikt", 11); }
    }
}
